...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp-Document-Data') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-document-data.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-document-data/issues') {
...
function checkReadmes() {
  ...
  if (!readmeJa.startsWith('# TurboWarp-Document-Data\n')) {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/document-data.js')) {
